...
public class OtherUtil {
    ...
    public static String getTextFromResource(String resourceName) {
        InputStream inputStream = ChatManager.class.getClassLoader().getResourceAsStream(resourceName);
        try {
    ...
    public static void downloadFile(String urlStr,String filePath,String fileName) throws IOException {
        ChatManager.plugin.getLogger().info("DOWNLOADING "+fileName+" FROM URL: "+urlStr);
        ...
        while((size = is.read(buf)) != -1) {
            ...
            if(progcess%5==0&&progcess!=lastSize){
                ChatManager.plugin.getLogger().info("DOWNLOADING "+fileName+" PROCESS:"+(100*nowSize/totalSize)+"%");
                lastSize=progcess;
            }
        }
        is.close();
        ...
        if(jar.exists())
            jar.delete();
        tmp.renameTo(jar);
        ChatManager.plugin.getLogger().info("DOWNLOAD "+fileName+" COMPLETE("+((System.currentTimeMillis()-startTime)/1000)+"s)");
    }
